...
var $author$project$PhotoGroove$init = function (_v0) {
	var activity = 'Initializing Pasta v' + $elm$core$String$fromFloat(1);
...
var $author$project$PhotoGroove$selectUrl = F2(
	function (url, status) {
		switch (status.$) {
			...
			case 'Loading':
				return $author$project$PhotoGroove$Loading;
			default:
				var errorMessage = status.a;
				return $author$project$PhotoGroove$Errored(errorMessage);
		}
	});
...
var $author$project$PhotoGroove$rangeSlider = F2(
	function (attrs, children) {
		return A3($elm$html$Html$node, 'range-slider', attrs, children);
	});
